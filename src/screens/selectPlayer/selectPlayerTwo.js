...
import { filterAndSortPlayersByRole, getPlayerIcon, getPlayerStatus, modifyName, modifyNameTwo, toastAlert } from '../../helper/utility';
...
const SelectPlayerTwo = () => {
  ...
  useEffect(() => {
    if (route?.params?.isEditMode || route?.params?.isCloneMode || route?.params?.shareTeam) {
  ...
  useEffect(() => {
    setTabs([
      `WK (${filterSelectedPlayer.wk.length})`,
      `BAT (${filterSelectedPlayer.bat.length})`,
      `AR (${filterSelectedPlayer.all.length})`,
      `BOWL (${filterSelectedPlayer.bowl.length})`,
    ]);
  }, [selectedPlayers, filterSelectedPlayer]);
  const getPlayersData = () => {
    if (route?.params?.isEditMode || route?.params?.isCloneMode) {
      ...
      if (activeTab.includes('BAT')) {
        ...
        return updatedDatanew.filter(player => player.playing_role == 'bat')
        ...
        return updatedDatanew.filter(player => player.playing_role == 'bowl')
          .sort(customSortRating).sort(customSort);;
        ...
        return updatedDatanew.filter(player => player.playing_role == 'wk')
          .sort(customSortRating).sort(customSort);;
        ...
        return updatedDatanew.filter(player => player.playing_role == 'all')
          .sort(customSortRating).sort(customSort);;
      }
    } else {
      const customSort = (a, b) => {
          ...
          if (saveTitle == 'PLAYERS') {
            ...
            if (newAllPlayer == 'high') {
              return allPlayers.filter(player => player.playing_role === 'bat')
                .sort((a, b) => b.fantasy_player_rating - a.fantasy_player_rating);
            } else {
              return allPlayers.filter(player => player.playing_role === 'bat')
                .sort((a, b) => a.fantasy_player_rating - b.fantasy_player_rating);
            }
  ...
  const getPlayerWk = () => {
    if (allPlayers.some(player => player.playing_role === 'wk' && player.playing11 !== undefined)) {
      return filterAndSortPlayersByRole(allPlayers, 'wk', saveTitle, newAllPlayer);
    } else {
      return allPlayers.filter(player => player.playing_role === 'wk')
        .sort((a, b) => a.first_name.localeCompare(b.first_name));
    }
  ...
  const getPlayerBAT = () => {
    if (allPlayers.some(player => player.playing_role === 'bat' && player.playing11 !== undefined)) {
      return filterAndSortPlayersByRole(allPlayers, 'bat', saveTitle, newAllPlayer);
    } else {
      return allPlayers.filter(player => player.playing_role === 'bat')
        .sort((a, b) => a.first_name.localeCompare(b.first_name));
    }
  ...
  const getPlayerAR = () => {
    if (allPlayers.some(player => player.playing_role === 'all' && player.playing11 !== undefined)) {
      return filterAndSortPlayersByRole(allPlayers, 'all', saveTitle, newAllPlayer);
    } else {
      return allPlayers.filter(player => player.playing_role === 'all')
        .sort((a, b) => a.first_name.localeCompare(b.first_name));
    }
  ...
  const getPlayerBOWL = () => {
    if (allPlayers.some(player => player.playing_role === 'bowl' && player.playing11 !== undefined)) {
      return filterAndSortPlayersByRole(allPlayers, 'bowl', saveTitle, newAllPlayer);
    } else {
      return allPlayers.filter(player => player.playing_role === 'bowl')
        .sort((a, b) => a.first_name.localeCompare(b.first_name));
    }
  ...
  const addPlayerInTeam = items => {
    console.log(items?.title, removedSpacesTeamsTitle[0], '====adasdas');
  ...
  const renderPlayerItem = ({ item }) => {
    ...
    const renderItemContent = () => (
      <Pressable
        style={[ !isSelected && styles.selectPlayerContainer, {
          flexDirection: 'row',
          justifyContent: 'space-between',
          alignItems: 'center',
          width: '100%',
          backgroundColor: !isSelected && "#343434",
          borderBottomWidth: !isSelected && selectedPlayers?.length == 11 ? 0 : 0.2
        }]}
        onPress={() => selectedPlayers?.length == 11 ? null : addPlayerInTeam(item)}>
        <TouchableOpacityView
          onPress={() => onDetail(item?.pid, item)}
  ...
  const onSubmit = (unannouncesSelect) => {
    ...
    if (unannouncesSelect?.length) {
      let newData = selectedPlayers.filter(player => !unannouncesSelect.some(unannouncedPlayer => unannouncedPlayer.pid === player));
      let newDatadetails = selectedPlayerDetails.filter(player => !unannouncesSelect.some(unannouncedPlayer => unannouncedPlayer.pid === player.pid));
      const totalFantasyRating = unannouncesSelect.reduce((total, player) => total + player.fantasy_player_rating, 0)
      let playerLengthOne = player.filter(player => !playerNew.some(newPlayer => newPlayer.pid === player.pid));
      let playerLengthTwo = playerTwo.filter(player => !playerTwoNew.some(newPlayer => newPlayer.pid === player.pid));
  ...
  const RenderWK = () => {
    return (
      <FlatList data={getPlayerWk()}
        renderItem={renderPlayerItem}
        keyExtractor={(index) => { index?.toString() }}
        contentContainerStyle={{ paddingHorizontal: universalPaddingHorizontal }}
        showsVerticalScrollIndicator={false} />
    )
  }
  const RenderBAT = () => {
    return (
      <FlatList data={getPlayerBAT()}
        keyExtractor={(index) => { index?.toString() }}
        contentContainerStyle={{ paddingHorizontal: universalPaddingHorizontal }}
        renderItem={renderPlayerItem}
        showsVerticalScrollIndicator={false} />
  ...
  return (
    <AppSafeAreaView hidden={false}>
      ...
      <RBSheet
        ...
        height={Unannounced?.length == 1 ? 230 : Unannounced?.length == 2 ? 330 : Unannounced?.length >= 3 ? 450 : 450}
        customStyles={{
          container: {
        ...
        <UnannouncedPlayer
          ...
          availableCredits={availableCredits}
          onClose={() => { refsheetUnannounced?.current?.close(), setOnClose(true) }} />
      </RBSheet>
...
export default SelectPlayerTwo;
